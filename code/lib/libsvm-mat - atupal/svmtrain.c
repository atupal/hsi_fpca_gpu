...
static void write_problem_dense(struct svm_problem*);
		...
		int nrhs, const mxArray *prhs[] )
{
    ...
    // cmd string
    int argc = 1;
    char cmd[CMD_LEN+200];
    char *argv[CMD_LEN/2+1];
    char cmdstring[2048]="D:\\±ÏÒµÉè¼Æ\\code\\lib\\CUDA-master\\binaries\\windows\\x64\\windows\\svm-train.exe ";
	...
	if(nrhs > 0 && nrhs < 4)
	{
        ...
        if(nrhs > 2)
        {
            ...
            idx = (int)strlen(cmdstring);
            if (cross_validation) 
                argv[argc] = "problem.txt > problem.model.txt";
            else
                argv[argc] = "problem.txt problem.model.txt";
		...
		if(cross_validation)
		{
            ...
            // get the vailidation accury of last line
            fd = fopen("problem.model.txt", "rb");
            /* now read that many bytes from the end of the file */
            fseek(fd, -maxlen, SEEK_END);
            
            fread(cmd, maxlen-1, 1, fd);
            if(fd) fclose(fd);
            ...
            lastline = strrchr(cmd, '\n');
            lastline += 1;
            //mexPrintf(lastline);
            sscanf(lastline, "Cross Validation Accuracy = %lf", &ac);
            //mexPrintf("%lf\n", ac);
            ptr[0] = ac;
		...
		else
		{
            ...
            mexPrintf("train svm\n");
            write_problem_dense(&prob); 
            
			//model = svm_train(&prob, &param);
            system(cmdstring);
            model = svm_load_model("problem.model.txt");
...
static void write_problem_dense(struct svm_problem *prob) {
    int i, j;
    FILE *fd, *test;
    fd=fopen("vector_length.txt", "r");
    test= fopen("problem.txt", "r");
    if (fd && test) {
        int l;
        fscanf(fd, "%d", &l);
        if (l == vector_length) return;
        fd=fopen("vector_length.txt", "w");
        fprintf(fd, "%d", vector_length);
        fclose(fd);
    } else {
        fd=fopen("vector_length.txt", "w");
        fprintf(fd, "%d", vector_length);
    ...
    fd = fopen("problem.txt", "w");
    for (i = 0; i < prob->l; ++ i) {
        fprintf(fd, "%d",(int) prob->y[i]);
        j = 0;
        while (prob->x[i][j].index != -1) {
            fprintf(fd, " %d:%lf", prob->x[i][j].index, prob->x[i][j].value);
            j += 1;
        }
        fprintf(fd, "\n");
    }
