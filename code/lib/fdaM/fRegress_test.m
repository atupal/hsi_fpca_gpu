...
%  function to scalar regression test

yvec0 = inprod(xfdcell{1}, getfd(betacell{1})) + ...
        inprod(xfdcell{2}, getfd(betacell{2})) + ...
        inprod(xfdcell{3}, getfd(betacell{3}));
yvec = yvec0 + randn(N,1).*sigma/10;

fRegressCell = fRegress(yvec, xfdcell, betacell);
